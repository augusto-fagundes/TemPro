...
export const SEED_PROVIDERS: SeedProvider[] = [
  {
    ...
    services: [
      ...
      },
      {
        ...
        desc: 'Montagem, fixação e ligação do controle de velocidade.',
        mode: 'Em domicílio',
        price: 'R$ 120',
      },
    ],
...
export function photosForProvider(
  provider: SeedProvider,
  index: number,
): { photoUrl?: string; photos?: string[] } {
  return {
    ...(index % 3 !== 2 && {
      photoUrl: placeholder(provider.category, index * 7 + 1, 240, 240),
    }),
    ...(index % 3 === 0 && {
      photos: Array.from({ length: 3 }, (_, n) =>
        placeholder(
          provider.category,
          ((index * 97 + n * 313 + 11) % 900) + 20,
          640,
          480,
        ),
      ),
